...
```text
/lark-chat-session 连接我和【同事】的私聊，检查新消息并按需回复。
```

群聊默认使用机器人身份；机器人不在群时，技能尝试以同一配置档案的用户身份邀请它入群。本人与机器人的私聊使用机器人身份，供用户从飞书控制当前智能体会话；与同事的私聊使用用户身份。多个机器人配置档案可用时，先询问使用哪个机器人。机器人群聊只把明确提及该机器人的消息交给会话；机器人单聊无需提及。用户身份会检查选定聊天的全部新消息，由智能体判断是否需要回应，并用回复消息上的 `DONE` 反应识别自己的输出。回复始终关联原消息，完成后在原消息上添加 `OK` 反应。智能体还可按需读取上下文、发送截图、录屏或文件，并与同事或其他智能体协作。
...
按所需身份执行相应检查；群聊需要邀请机器人时，同一配置档案还需用户身份。应用密钥按 `lark-cli` 的交互引导或标准输入提供，不放进命令参数、聊天或仓库。机器人需要加入目标群，并在飞书后台获得读写消息、表情反应和接收消息事件的权限；用户身份所需的授权以实际接口为准。部分外部群的历史读取受平台限制，无法读取时插件会报告该聊天不支持可靠持续响应。

### 连接与停止

智能体用所选身份查找目标聊天，先确认聊天历史可读，再把**一个聊天**绑定到当前 Codex 任务或 Claude Code 会话。本机允许不同会话同时绑定不同聊天；同一聊天只有一个活动绑定，同一会话也只连接一个聊天。更换聊天或接管已有绑定需要先停止原唤醒并明确接管。群聊查找、单聊发现、历史读取、消息回复、附件和其他飞书操作都直接调用 `lark-cli`，并且每条命令显式传配置档案和身份。

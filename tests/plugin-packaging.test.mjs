...
describe("skill-only plugin packaging", () => {
  ...
  it("ships the session script and teaches direct profile-bound lark-cli calls", () => {
    ...
    const setup = readFileSync("plugins/lark-connect/skills/lark-connect-setup/SKILL.md", "utf8");
    ...
    assert.match(setup, /lark-cli config init/);
    assert.doesNotMatch(skill + setup, /lark_connect_\w+|DAEMON_NOT_RUNNING|curiosea-lark-connect@latest/);
  });
});

...
- Claude Code：使用 background shell 运行 `npx -y curiosea-lark-connect@latest wait --agent-session-id <当前 Claude Code session ID> --timeout-ms 300000`。命令结束后 Claude Code 会唤醒当前智能体；如果输出里有消息，就处理、回复、ack，然后再做一次 1 分钟 `wait`。如果输出里没有消息，再启动下一轮 background shell。

...
这三个文件是随技能分发的单用途脚本，不注册全局命令，也不代理飞书全部接口。调用者使用 `node <技能目录>/scripts/<文件>.mjs <动作> ...`；标准输出只返回一份 JSON，诊断写入标准错误。成功退出码为 0；输入、连接冲突、上游失败或发送结果不确定时使用非零退出码，并返回稳定的 `code`。除首次连接外，脚本从检查点读取配置档案和身份，每次调用 `lark-cli` 仍显式传入 `--profile` 和 `--as`，不使用机器默认身份。

| 脚本动作 | 输入 | 成功输出 | 可能的副作用 |
|---|---|---|---|
| `connect.mjs start` | 配置档案、身份、聊天标识、运行时、会话标识；接管时加 `--replace` | 连接摘要及新的 `generation` | 验证目标可读后写入单连接检查点 |
| `connect.mjs status` | 无；可选会话标识用于核对归属 | 当前连接摘要、扫描时间、待处理数量；没有连接时返回 `stopped` | 无 |
| `connect.mjs stop` | 会话标识、`generation` | `stopped` 及原连接摘要 | 停止当前连接；技能撤销相应运行时唤醒 |
| `check.mjs run` | 会话标识、`generation`；可选单批返回数量 | 本次扫描范围、完整性、候选消息、是否还有待处理候选 | 完整扫描后更新检查点并记录候选标识 |
| `check.mjs ignore` | 会话标识、`generation`、原消息标识 | 该消息的 `ignored` 结果 | 记录智能体已判断无需回复 |
| `reply.mjs send` | 会话标识、`generation`、原消息标识、回复内容文件或纯文本 | 各部分的回复标识及确认状态 | 按原消息回复，全部成功后添加 `OK` 反应 |
| `reply.mjs resolve` | 会话标识、`generation`、原消息标识；人工核实的“已发送回复标识”或“确认未发送” | 核实后的消息状态及下一步 | 只修复不确定结果，不自行猜测是否已发送 |

通用失败形态为 `{"ok":false,"code":"CHAT_UNREADABLE","message":"...","retryable":false}`；检查失败额外给出 `scanAdvanced:false`，发送不明额外给出 `mayHaveSent:true`。错误不回显凭据和完整聊天内容。
...
- **职责与行为**：按原消息标识记录 `pending`、`ignored`、`sending`、`replied`、`confirmed` 或 `needs_review`。`ignored` 记录用户身份下无需回应的判断；`sending` 先于飞书写入，`replied` 按回复部分保存实际回复标识列表，`confirmed` 表示全部回复成功且 `OK` 反应成功。旧记录仅在对应历史窗口不再需要时清理。
...
- **有效状态规则**：同一原消息的同一回复部分只能进入一次发送中的状态；未确认全部发送成功前不得标记完成；已成功的部分和补确认不得再次发送（每次状态转换时）。
...
- **输入**：`start --profile <名称> --as bot|user --chat-id <标识> --runtime codex|claude-code --session-id <稳定标识> [--replace]`；`status` 无必需参数；`stop --session-id <标识> --generation <代次>`。接管必须显式传 `--replace`，不根据聊天相同自动覆盖。
- **输出**：`{"ok":true,"connection":{"status":"active","profile":"team-bot","as":"bot","chatId":"oc_...","runtime":"codex","sessionId":"...","generation":"...","startedAt":"..."}}`；`status` 另给扫描时间与待处理数量，`stop` 返回 `status:"stopped"`。不输出凭据。连接前读历史验证失败则不写活动连接；连接开始前先记录起始时间，再做聊天读取验证，验证期间产生的新消息在首次检查时仍可发现，既有历史只用作上下文。
...
- **输入**：`run --session-id <标识> --generation <代次> [--batch-size 20]`，或 `ignore --session-id <标识> --generation <代次> --message-id <原消息标识>`；不接受另一个聊天标识覆盖连接。
- **输出**：`run` 返回 `{"ok":true,"scan":{"status":"complete","from":"...","through":"...","hasMorePending":false},"messages":[{"messageId":"om_...","chatId":"oc_...","sender":{"id":"ou_...","type":"user"},"createdAt":"...","messageType":"text","content":"...","mentions":[],"replyToMessageId":null,"threadId":null,"resources":[]}]}`。消息内容仅在本次输出出现，不写入状态文件；一次最多返回指定批量，未决候选下次仍可见。`ignore` 返回 `{"ok":true,"messageId":"om_...","outcome":"ignored"}`，不加完成反应。
...
技能使用 `lark-cli` 读取原消息、上下文、资源与成员，再由 `reply.mjs` 执行有状态写入。脚本按内容文件中的顺序处理一项或多项回复，每项先记录 `sending`，用原消息和部分序号生成稳定幂等键并显式指定身份，调用 `lark-cli im +messages-reply`；确认返回标识后保存该部分的回复标识，全部完成后再调用反应接口并保存 `confirmed`。若任一发送结果不确定，保留 `needs_review`，不得在超过上游幂等窗口后自动重发；会话主人核查飞书原消息后才能恢复。若只是确认反应失败，从 `replied` 状态补确认，不再发送回复。图片、视频与文件使用上游回复命令的对应参数；脚本把本地绝对路径安全转换成上游要求的工作目录相对路径，视频和封面位于不同目录时用临时目录准备文件。额外提及目标通过上游支持的文本或富文本形态传入，技能先读成员并验证标识。
...
- **输入**：`send --session-id <标识> --generation <代次> --message-id <原消息标识> --payload-file <JSON 文件>`；内容文件为 `{"replyInThread":false,"parts":[{"kind":"text","text":"已处理","mentions":[{"openId":"ou_...","name":"协作者"}]},{"kind":"image","path":"/本机/截图.png"}]}`。每项 `kind` 为 `text`、`markdown`、`image`、`file` 或 `video`；媒体使用 `path`，视频还需 `coverPath`。普通文本无需先建文件时可用互斥的 `--text <内容>`。只允许检查点绑定的身份与聊天；内容文件不被复制进状态文件。
- **输出**：成功时返回 `{"ok":true,"messageId":"om_...","outcome":"confirmed","replyIds":["om_reply1","om_reply2"],"acknowledged":true}`。确认反应失败时非零退出，返回 `{"ok":false,"code":"ACK_FAILED","outcome":"replied","replyIds":["om_reply1"],"acknowledged":false}`；发送结果不明时返回 `{"ok":false,"code":"SEND_UNCERTAIN","outcome":"needs_review","partIndex":0,"mayHaveSent":true}`。
- **人工恢复输入**：`resolve --session-id <标识> --generation <代次> --message-id <原消息标识> --part-index <序号> --sent-reply-id <已核实的回复标识>`，或将最后一项改为 `--confirmed-not-sent`。前者须向飞书核对回复确属当前聊天和原消息，后者须有人明确确认未发送；返回修复后的状态及可继续的下一步。
- **错误与副作用**：重复调用已确认消息返回既有结果；`replied` 只补确认，已成功的回复部分不重发；`sending` 结果不明时不自动重发。无需回复的普通用户消息通过 `check.mjs ignore` 记录，不调用本接口。

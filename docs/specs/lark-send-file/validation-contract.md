# Validation Contract — Lark Send File (验收契约 — 飞书文件发送工具)
...
| Range (范围) | VAL ids |
|---|---|
| MCP 工具暴露与转发 | VAL-FILE-001 |
| 绑定群文件发送与回复 | VAL-FILE-002 |
| 未绑定错误 | VAL-FILE-003 |
...
### VAL-FILE-001
- **Behavior (行为)**: MCP 工具列表包含 `lark_connect_send_file`，且调用时会把 `agentSessionId`、`filePath`、`fileName`、`replyToMessageId` 和 `replyInThread` 传给本地 daemon。
- **Tool (工具)**: unit-test
- **Evidence (判据)**: MCP 测试能观察到工具列表包含该工具，工具调用返回新飞书消息标识。

### VAL-FILE-002
- **Behavior (行为)**: 已绑定 session 调用文件发送工具时，文件只发送到该 session 绑定的飞书群；提供 `replyToMessageId` 时发送结果是对该消息的回复。
- **Tool (工具)**: integration-test
- **Evidence (判据)**: 本地 daemon 测试注入出站客户端后，能观察到目标 `chatId`、本地文件路径、最终文件名和回复参数，并返回新消息标识。

### VAL-FILE-003
- **Behavior (行为)**: 未绑定 session 调用文件发送工具时不触发飞书发送，并返回 `SESSION_NOT_BOUND` 错误。
- **Tool (工具)**: integration-test
